...
test('A fresh media-element output is never primed with silence: the first thing it renders must be voice',async()=>{
 // Priming with silence (2026-09-19) made the phone's echo cancellation stop covering the room's voice.
 const s=setup();const {context}=mediaOutput(s);
 ...
 assert.equal(made.length,0,'nothing is scheduled into the sink before the first utterance');
 assert.equal(s.voice.health().events.some(e=>e.kind==='prime'),false);
});
