    ...
    <div id="language-rows">
      ...
      {rows.map((row) => (
        <div className="language-row" key={row.language}>
          ...
          />
          <Button id={`preview-${row.language}`} variant="ghost" size="icon" aria-label={`Probar voz · ${row.label}`} title={`Probar voz · ${row.label}`} onClick={() => void window.sidevoiceActions?.previewVoice(row.language)}>▶</Button>
        </div>
      ))}
    </div>
  );
