...
const replayMarks=new Map();
const REPLAY_NOTES={queued:'Repitiendo lo que no oíste',playing:'Repitiendo lo que no oíste',
 done:'Repetido al volver',cancelled:'Repetición cancelada',
 gone:'No se pudo repetir · la sala ya no tiene ese audio'};
function replayNote(r){return r.role==='assistant'&&REPLAY_NOTES[replayMarks.get(r.segment)]||''}
function markReplay(id,state){if(!id||!replayMarks.has(id))return;replayMarks.set(id,state);renderHistory()}
function noteReplay(d){
 for(const item of d.replies||[])if(item?.history_id)replayMarks.set(item.history_id,'queued');
 for(const item of d.skipped||[])if(item?.history_id)replayMarks.set(item.history_id,'gone');
 renderHistory();
}
...
function cancelPendingReplays(){
 let changed=false;
 for(const [id,state] of replayMarks)if(state==='queued'||state==='playing'){replayMarks.set(id,'cancelled');changed=true}
...
// One level, not a knob: the operator tried the slider and said it did not earn its place. This is the
// value that carries on a phone speaker in a car without ever competing with a voice.
...
// The harness's own state, when it has one: the dots go on before any receipt and stay until it says it is
// done. What the conversation publishes cannot contradict it — an acknowledgement is not the end of a turn.
const HARNESS_TURN=':harness';
function markWorking(){const id=(sessionId||'')+HARNESS_TURN;if(workingTurn!==id){workingTurn=id;renderHistory()}resumeBed('working')}
// The bed belongs to the silence between us. It goes while either of us speaks and comes back when the room
// is quiet again, for as long as the dots are on; a browser with the sound off never hears any of it.
function resumeBed(reason){
 if(!workingTurn||presenceTurn||userLive||botLive||activeSpeech)return;
 const options=presenceOptions();
 if(!options||!ws||!window.roomVoice?.startPresence)return;
 if(window.roomVoice.startPresence({...options,reason}))presenceTurn=workingTurn;
}
/* One rule, asked at every moment that can change the answer: while the dots are on and nothing else is
 * sounding, the bed sounds. Anything else — either of us speaking, a reply playing — silences it, and when
 * that passes it comes back on its own. Called from the state changes rather than polled. */
function syncBed(reason){
 if(workingTurn&&!userLive&&!botLive&&!activeSpeech)resumeBed(reason);
 else if(presenceTurn){presenceTurn=null;window.roomVoice?.stopPresence?.(reason)}
}
function stopPresence(reason,turn){
 if(workingTurn&&workingTurn.endsWith(HARNESS_TURN)&&!['finished','focus_changed','connection_lost','disconnected'].includes(reason)){
  if(presenceTurn){presenceTurn=null;window.roomVoice?.stopPresence?.(reason)}
  return;   // the harness says it is working; only the harness, or the call ending, says otherwise
 }
...
function message(raw){let m;try{m=JSON.parse(raw)}catch{return}const t=m.type,d=m.data||{};observeLatencyEvent(t,d);if(t==='voice-transcribe'){if(d.session_id===sessionId)window.roomTranscription.transcribe(d);return}if(t==='voice-speech'){receiveBrowserSpeech(d);return}if(t==='voice-speech-audio'){receiveServerSpeech(d);return}if(t==='voice-replay'&&d.session_id===sessionId){noteReplay(d);return}if(t==='voice-cancel'&&d.session_id===sessionId){roomRevision=Math.max(roomRevision,d.revision);stopPresence('audio_cancelled');cancelPendingReplays();cancelBrowserSpeech();return}if(t==='voice-user-turn'&&d.phase==='started'){roomRevision=Math.max(roomRevision,d.revision);stopPresence('new_turn');cancelPendingReplays();cancelBrowserSpeech()}if(t==='voice-catchup-turn'&&d.session_id===sessionId){/* a message from the gap: its own bubble, with this browser's own clock, and nothing of the turn that may be open now */
...
function disconnect(){
 ...
 disarmGapBuffer();replayMarks.clear();
...
async function joinRoom(epoch,context){
 ...
 try{session=await openSession(socket,{conversation:rememberedThread(),sessions:rememberedSessions(),settings:voicePreferences,transcription:context.sttRuntime,...(traceparent?{telemetry:{traceparent}}:{})})}
 ...
 sessionId=session.session_id;roomRevision=0;rememberSession(sessionId);
...
async function receiveBrowserSpeech(d,cloud=false){const receivedAt=latencyNow();if(d.session_id!==sessionId)return;if(d.thread_id!==targetId())await refresh();if(d.session_id!==sessionId||d.thread_id!==targetId()||d.revision<roomRevision)return;roomRevision=d.revision;stopPresence(d.final===false?'progress':'reply',(d.session_id||sessionId)+':user-turn:'+(d.reply_revision??d.revision));stopPreview();cancelBrowserSpeech();activeSpeech=d;add('assistant',d.text,'voice:'+d.utterance_id,d.thread_id,{history_id:d.history_id,session:d.session_id,revision:d.revision});const receipt=status=>post('/api/presentation/browser-receipt',{session_id:d.session_id,revision:d.revision,utterance_id:d.utterance_id,status,...(status==='playing'?{timings_ms:browserLatency(d,receivedAt)}:{})});try{await window.roomVoice[cloud?'playEncoded':'speak'](d,text=>$('live').textContent=text,()=>{d.started=true;botLive=true;if(d.replay)markReplay(d.history_id,'playing');renderHistory();live();receipt('playing').catch(()=>{})},range=>updateKaraoke(d,range));if(activeSpeech!==d){endTurnTrace(d,'superseded');return}clearKaraoke(d);activeSpeech=null;renderHistory();botLive=false;syncBed('reply_over');live();endTurnTrace(d,'played');if(d.replay)markReplay(d.history_id,'done');await receipt('playback_finished')}catch(e){if(activeSpeech!==d){endTurnTrace(d,'superseded');return}clearKaraoke(d);activeSpeech=null;renderHistory();botLive=false;syncBed('reply_over');live();endTurnTrace(d,e.name==='AbortError'?'interrupted':'failed');if(d.replay)markReplay(d.history_id,'cancelled');if(e.name!=='AbortError'){setRoomError((cloud?'Audio de ElevenLabs: ':'Voz del navegador: ')+e.message);receipt('failed').catch(()=>{})}}}
...
const MIC_KEYS=['turn_end_mode','user_speech_timeout','smart_turn_min_silence','smart_turn_max_silence'];
