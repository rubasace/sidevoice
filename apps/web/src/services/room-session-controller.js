...
async function startCapture(socket,session){if(!micSource)throw Error('No se pudo capturar el micrófono');
 ...
 const node=new AudioWorkletNode(context,'mic-capture',{numberOfInputs:1,numberOfOutputs:1,outputChannelCount:[1],channelCount:1,channelCountMode:'explicit',processorOptions:{sampleRate:session.sample_rate}});captureNode=node;node.port.onmessage=e=>{if(captureNode===node&&ws===socket&&socket.readyState===WebSocket.OPEN&&micTrack()?.enabled){if(!(micPauseDuringPlayback&&botLive))socket.send(e.data)}};source.connect(node);node.connect(context.destination)/* reachable from the destination so it keeps running; its output stays silent */}
function disconnect(){
...
$('settings-open').onclick=async()=>{try{
 ...
 const mic=micSettings();for(const key of MIC_KEYS)$(key.replaceAll('_','-')).value=mic[key]??p[key];$('mic-pause-during-playback').checked=micPauseSetting();
...
// Browser-only: on a phone speaker the reply leaks back into the microphone and opens a false turn; this device can keep its microphone quiet while a reply plays (no voice interruption then: use the stop button).
let micPauseDuringPlayback=false;
function micPauseSetting(){try{return !!JSON.parse(localStorage.getItem('sidevoice.mic')||'{}')?.pause_during_playback}catch{return false}}
// How this device's turns are detected lives on this device; the room only keeps defaults for devices that saved nothing.
function micSettings(){let stored={};try{stored=JSON.parse(localStorage.getItem('sidevoice.mic')||'{}')||{}}catch{}const mic={};for(const key of MIC_KEYS){const value=stored[key]??voicePreferences?.[key];if(value!==undefined&&value!==null&&value!=='')mic[key]=key==='turn_end_mode'?String(value):Number(value)}return mic}
function storeMicSettings(mic){try{localStorage.setItem('sidevoice.mic',JSON.stringify(mic))}catch{}micPauseDuringPlayback=!!mic.pause_during_playback}
