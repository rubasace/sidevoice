...
$('settings-open').onclick=async()=>{try{
 ...
 const mic=micSettings();for(const key of MIC_KEYS)$(key.replaceAll('_','-')).value=mic[key]??p[key];
...
const MIC_KEYS=['turn_end_mode','user_speech_timeout','smart_turn_max_silence','vad_confidence','vad_min_volume'];
...
function storeMicSettings(mic){try{localStorage.setItem('sidevoice.mic',JSON.stringify(mic))}catch{}}
