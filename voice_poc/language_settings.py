...
class LanguageVoice(BaseModel):
    ...
    model: Literal['inherit', 'kokoro'] = 'inherit'
    voice: str = 'inherit'
...
class LanguageSettings(BaseModel):
    ...
    default_model: Literal['kokoro'] = 'kokoro'
    spanish_model: Literal['inherit', 'kokoro'] = 'inherit'
    english_model: Literal['inherit', 'kokoro'] = 'inherit'
    ...
    def supported_voices(self):
        ...
        for language, override in self.language_overrides.items():
            ...
            if override.voice != 'inherit' and override.voice not in {v[0] for v in LANGUAGES[language]['voices']}:
...
def resolve_voice(settings, language=None):
    ...
    if voice == 'inherit':
        ...
        if voice not in {v[0] for v in LANGUAGES[language]['voices']}:
            voice = LANGUAGES[language]['voices'][0][0]
    return {'model': settings.default_model if model == 'inherit' else model,
